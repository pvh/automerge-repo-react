...
export function textMark(doc: Automerge.Doc<unknown>, objId: string, range: string, name: string, value: string) {
  (Automerge as any).getBackend(doc).mark(objId, range, name, value)
}

export function textInsertAt(doc: Automerge.Doc<unknown>, objId: string, position: number, value: string) {
  const ins = (Automerge as any).getBackend(doc).splice(objId, position, 0, value)
  return ins
}

export function textDeleteAt(doc: Automerge.Doc<unknown>, objId: string, position: number, count = 1) {
  return (Automerge as any).getBackend(doc).splice(objId, position, count, '')
}

export function textInsertBlock(doc: Automerge.Doc<unknown>, objId: string, position: number, type: string, attributes: { [key: string]: unknown } = {}) {
...
export function textGetBlock(doc: Automerge.Doc<unknown>, objId: string, position: number) {
