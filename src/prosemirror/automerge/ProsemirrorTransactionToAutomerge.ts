...
export const prosemirrorTransactionToAutomerge = <T>(
  ...
  // Remove the steps from the transaction object.
  // This is an unsupported way of doing this (hence the any), but there is
  // often other data on the transaction that Automerge doesn't handle so we need
  // to let Prosemirror deal with whatever else is going on (like cursor positions.)
  (transaction as any).steps = [];
};
